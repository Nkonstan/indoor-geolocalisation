...
import shutil
import gc
from materobot_app import MATERobotModel
ImageFile.LOAD_TRUNCATED_IMAGES = True
...
def calculate_material_proportions(model, result) -> str:
    ...
    # Convert tensor to 2D numpy array and move to CPU
    sem_seg_2D = result.pred_sem_seg.data.squeeze().cpu().numpy()
...
def material_pred(im_path):
    ...
    try:
        ...
        pred_labels = result[0].pred_sem_seg.data.squeeze().cpu().unique().tolist()

        # Calculate material proportions
        info_mat = calculate_material_proportions(materobot_model.model, result[0])
        ...
        mask_path = show_result_pyplot(materobot_model.model, im_path, result[0], show=False, opacity=0.8,
                                       out_file=(str(uuid.uuid4()) + '.png'),
                                       save_dir="/app/static")

        # Clean up result tensors
        del result, pred_labels
